...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (err && typeof err === 'object' && 'code' in err && (err as any).code === 'P2002') {
      return ok({ ignored: 'event_duplicate' })
    }
    throw err
  }
  ...
  if (resend && !disableArtistEmails) {
    for (const entry of artistGroups.values()) {
      ...
      try {
        const rows = entry.items.map((it, idx) => (
          <tr key={`${it.artworkSlug}-${idx}`}>
            <td style={{ padding: '4px 8px' }}>{it.qty}×</td>
            <td style={{ padding: '4px 8px' }}>
              {it.title}
              {it.variantLabel ? ` — ${it.variantLabel}` : ''}
            </td>
            <td style={{ padding: '4px 8px', textAlign: 'right' }}>{fmtCurrency(it.unitPrice)}</td>
            <td style={{ padding: '4px 8px', textAlign: 'right' }}>{fmtCurrency(it.lineTotal)}</td>
          </tr>
        ))

        const links =
          siteUrl && entry.items.length
            ? entry.items.map((it, idx) => (
                <React.Fragment key={`${it.artworkSlug}-${idx}`}>
                  {idx > 0 ? ', ' : null}
                  <a href={`${siteUrl}/artworks/${it.artworkSlug}`} style={{ color: '#2563eb' }}>
                    {it.title}
                  </a>
                </React.Fragment>
              ))
        ...
        const html = await renderEmail({
          ...
          intro: <p>Bonjour {entry.artistName},</p>,
          children: (
            <>
              ...
              <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: 12 }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: 'left', padding: '4px 8px' }}>Œuvre</th>
                    <th style={{ textAlign: 'right', padding: '4px 8px' }}>PU</th>
                    <th style={{ textAlign: 'right', padding: '4px 8px' }}>Total</th>
                  </tr>
                </thead>
                <tbody>{rows}</tbody>
              </table>
              {links ? <p style={{ marginTop: 12 }}>Liens : {links}</p> : null}
              <p style={{ marginTop: 12 }}>Vous recevrez les détails logistiques très bientôt. Merci !</p>
            </>
          ),
          footer: <p>Besoin d’aide ? Réponds simplement à cet email.</p>,
        })
  ...
  try {
    ...
    if (resend && adminTo) {
      const adminRows = (orderRecord.items ?? []).map((it) => {
      ...
      const html = await renderEmail({
        title: 'Nouvelle commande confirmée',
        intro: <p>Bonjour l’équipe Point Bleu,</p>,
        children: (
          <>
            <p style={{ margin: '12px 0' }}>
              <strong>Commande :</strong> {orderRecord.id}
            </p>
            <p style={{ margin: '12px 0' }}>
              <strong>Session Stripe :</strong> {session.id}
            </p>
            <p style={{ margin: '12px 0' }}>
              <strong>Client :</strong> {email || '(inconnu)'}
            </p>
            <p style={{ margin: '12px 0' }}>
            ...
            <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: 16 }}>
              <thead>
                <tr>
                  <th style={{ textAlign: 'left', padding: '4px 8px' }}>Artiste</th>
                  <th style={{ textAlign: 'left', padding: '4px 8px' }}>Œuvre</th>
            ...
            {missingContacts.size ? (
              <p style={{ marginTop: 12 }}>
                <strong>Attention :</strong> aucun email de contact pour {Array.from(missingContacts).join(', ')}.
              </p>
            ) : null}
            {artistSendFailures.length ? (
              <p style={{ marginTop: 12 }}>
                <strong>Envois artistes en erreur :</strong>{' '}
                {artistSendFailures.map((f, idx) => (
                  <React.Fragment key={`${f.artist}-${idx}`}>
                    {idx > 0 ? ', ' : null}
                    {`${f.artist} (${f.to})`}
                  </React.Fragment>
                ))}
            ...
            {txResult.missing.length ? (
              <p style={{ marginTop: 12 }}>
                <strong>Articles ignorés (variant introuvable) :</strong>{' '}
                {txResult.missing
        ...
        footer: <p>Besoin d’aide ? Répondez simplement à cet email.</p>,
      })
