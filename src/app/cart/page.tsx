...
export default function GlobalCartDrawer() {
  ...
  // stop propagation for any interaction inside the panel
  const stop = (e: React.SyntheticEvent) => {
    e.stopPropagation()
  }

  return (
    // OVERLAY: close only if click directly on the overlay (not on children)
    <div
                ...
                <li key={i.key} className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between">
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); remove(i.key) }} className="text-xs text-neutral-500 hover:text-accent">Retirer</button>
                    </div>
                    <div className="mt-0.5 text-xs text-neutral-500">
                      {i.format ? `Format — ${i.format.label}` : 'Format standard'}
                    </div>
                    <div className="mt-2 flex items-center gap-2" onClick={stop} onMouseDown={stop}>
                      <label className="text-xs text-neutral-500" htmlFor={`qty-${i.key}`}>Qté</label>
                      <input
                        id={`qty-${i.key}`}
                        type="number"
                        min={1}
